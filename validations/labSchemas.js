...
const updateLabOrderTestSchema = z.object({
  ...
  machineUsed: z
    .string()
    .max(100, "Machine used must not exceed 100 characters")
    .optional(),
...
const createParameterSchema = z.object({
  ...
  referenceRange: z
    .string()
    .min(1, "Reference range is required")
    .max(200, "Reference range must not exceed 200 characters"),
  maleRange: z
    .string()
    .max(200, "Male range must not exceed 200 characters")
    .optional(),
  femaleRange: z
    .string()
    .max(200, "Female range must not exceed 200 characters")
    .optional(),
  childRange: z
    .string()
    .max(200, "Child range must not exceed 200 characters")
    .optional(),
  adultRange: z
    .string()
  ...
  methodology: z
    .string()
    .max(200, "Methodology must not exceed 200 characters")
    .optional(),
  instrumentUsed: z
    .string()
    .max(100, "Instrument must not exceed 100 characters")
    .optional(),
});
  ...
  results: z
    .array(
      z.object({
        ...
        technicalRemarks: z
          .string()
          .max(1000, "Technical remarks must not exceed 1000 characters")
          .optional(),
...
const updateResultSchema = z.object({
  ...
  clinicalRemarks: z
    .string()
    .max(1000, "Clinical remarks must not exceed 1000 characters")
    .optional(),
  instrumentUsed: z.string().optional(),
  methodUsed: z.string().optional(),
  dilutionFactor: z.number().positive().optional(),
...
const authorizeResultsSchema = z.object({
  resultIds: z
    .array(z.string().min(1, "Result ID is required"))
  ...
  bulkRemarks: z
    .string()
    .max(1000, "Bulk remarks must not exceed 1000 characters")
    .optional(),
});
...
const bulkUpdateStatusSchema = z
  .object({
  ...
  .refine(
    (data) => {
      const hasIds =
        data.orderIds?.length || data.testIds?.length || data.resultIds?.length;
      return hasIds > 0;
    },
    {
