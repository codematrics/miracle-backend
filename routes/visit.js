...
} = require("../controllers/visit/visit");

const router = express.Router();

router.get("/", listVisitController);
...
router.get("/", validate(visitQuerySchema, "query"), async (req, res) => {
  ...
  try {
    ...
    if (hasPatientFilters) {
      ...
      const pipeline = [
        ...
        // Apply filters
        {
          $match: {
            ...
            ...(mobileNo && {
              "patient.mobileNo": { $regex: mobileNo, $options: "i" },
            }),
            ...(patientName && {
              "patient.patientName": { $regex: patientName, $options: "i" },
            }),
            ...(uhid && { "patient.uhid": { $regex: uhid, $options: "i" } }),
            ...(doctorName && {
              visitingdoctor: { $regex: doctorName, $options: "i" },
            }),
            ...(visitType && {
              visittype: { $regex: visitType, $options: "i" },
            }),
      ...
      const totalPages =
        all === "true" ? 1 : Math.ceil(total / parseInt(limit));
      ...
      const doctorQuery = doctorName
        ? { visitingdoctor: { $regex: doctorName, $options: "i" } }
        : {};
      const visitTypeQuery = visitType
        ? { visittype: { $regex: visitType, $options: "i" } }
        : {};
      const refbyQuery = refby
        ? { refby: { $regex: refby, $options: "i" } }
        : {};
          ...
          errors: [
            {
              field: "doctorId",
              message:
                "Doctor with this Employee ID does not exist or is inactive",
            },
          ],
        });
      }
